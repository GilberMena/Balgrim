...
export async function mercadopagoCheckout(req, res) {
  const { orderId, customer } = req.body;
  if (!orderId) {
    throw new AppError("orderId is required.", 400);
  }
  ...
  const checkout = await createMercadoPagoCheckout(order, customer || {});
  res.json({ ok: true, checkout });
}

export async function addiCheckout(req, res) {
  const { orderId, customer } = req.body;
  if (!orderId) {
    throw new AppError("orderId is required.", 400);
  }
  ...
  const checkout = await createAddiCheckout(order, customer || {});
  res.json({ ok: true, checkout });
}
